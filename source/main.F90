program polar_sim
    ...
    use initial_ops,       only : initialize_flow_fields
    use output_writer,     only : write_primitive_tecplot, print_time_info
    use time_step_control, only : compute_global_timestep
    use flux_ops,          only : compute_all_fluxes
    use field_update_ops,  only : update_conserved_fields
    ...
    do while (time%t < time%t_end .and. time%step < time%max_steps)
        ...
        call update_conserved_fields(mesh, cons, flux, time%dt)
